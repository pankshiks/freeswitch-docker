...
stack_sounds_get () {
    ...
    read -r -p "please choose sounds install directory [$(pwd)]: " SOUNDS_DIR
    SOUNDS_DIR=${SOUNDS_DIR:-$(pwd)}
    ...
    for SOUND in "${SOUNDS[@]}"; do
        if [ ! -f /tmp/sounds/"$SOUND" ]; then
            sleep 5  # Wait for 5 seconds before downloading
    ...
    ls -la "$SOUNDS_DIR" || { printf "error on line: %s\n" "$LINENO" && exit; }
